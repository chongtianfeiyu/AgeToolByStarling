package feathers.utils
{
	...
	import feathers.ext.ScaleButton;
	...
	public class LayoutUitl
	{
		 ...
		 * 创建按钮
		 * */
		public function createButton(name:String):ScaleButton{
			var skin:Sprite = new Sprite();
			buildLayout(name,skin);
			var btn:ScaleButton = new ScaleButton();
			var arr:Array = [];
			var text:TextField;
			for(var i:int=0;i<skin.numChildren;i++)
			{
				if(skin.getChildAt(i) is Image)
				{
					arr.push(skin.getChildAt(i));
				}
				else if(skin.getChildAt(i) is TextField)
				{
					text = (skin.getChildAt(i)) as TextField;
				}
			}
			...
			{
				btn.downSkin = arr[0];
				btn.hoverSkin =  btn.upSkin = arr[1];
			}
			else
			{
				btn.defaultIcon = arr[0];
			...
			if(text)
			{
				btn.labelFactory = function():ITextRenderer
				{
					...
					tr.textFormat = new TextFormat(text.fontName,text.fontSize,text.color);
					return tr;
				}
				btn.label = text.text;
			}
